pipeline {
    parameters {
        ...
        string(name: 'DEVELOPMENT_NAMESPACE',      description: 'Development Namespace',    defaultValue: 'smartcity-sragen-dev')

        string(name: 'DOCKER_DEV_REGISTRY_URL',    description: 'docker registry',          defaultValue: 'docker-registry-default.vsan-apps.playcourt.id')
        string(name: 'DOCKER_STAGING_REGISTRY_URL',description: 'docker registry',          defaultValue: 'docker-registry-default.vsan.apps.playcourt.id')
        ...
        string(name: 'DOCKER_IMAGE_NAME',          description: 'Docker Image Name',        defaultValue: 'dashboard-smartcity-sragen')
    }
    agent none
    options {
        skipDefaultCheckout()
    ...
    stages {
        ...
        stage('Deploy to DEV') {
            environment {
                KUBE_DEV_TOKEN = credentials('VSAN_OC_REGISTRY_TOKEN')
            }
            when {
                branch 'development'
            }
